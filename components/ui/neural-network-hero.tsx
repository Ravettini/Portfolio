...
/**
 * Neural Network Hero — /components/ui (shadcn structure).
 * Tailwind + TypeScript. Shader CPPN con Three.js + @react-three/fiber v9.
 * Material custom (THREE.ShaderMaterial) en lugar de shaderMaterial de drei por compatibilidad con React 19.
 *
...
const fragmentShader = `
  ...
  void main() {
    ...
    float t = uReducedMotion > 0.5 ? 0.0 : iTime;
    vec4 raw = cppn_fn(uv, 0.1 * sin(0.3 * t), 0.1 * sin(0.69 * t), 0.1 * sin(0.44 * t));
    // Tono azulado / navy (no violeta): reducir R, reforzar B/G para identidad dark blue
    float r = raw.x * 0.12;
    float g = raw.y * 0.22;
    float b = 0.15 + raw.z * 0.55;
    gl_FragColor = vec4(r, g, b, 1.0);
  }
`;

class CPPNShaderMaterialImpl extends THREE.ShaderMaterial {
  constructor() {
    super({
      uniforms: {
        iTime: { value: 0 },
...
extend({ CPPNShaderMaterial: CPPNShaderMaterialImpl });
...
function ShaderBackground({ reducedMotion }: { reducedMotion: boolean }) {
  ...
  return (
    <div
      ref={canvasRef}
      className="bg-[#05070a] absolute inset-0 -z-10 h-full"
...
export default function Hero({
  ...
  useGSAP(
    () => {
      ...
      const applyAnimations = () => {
        if (reducedMotion) {
          gsap.set([badgeRef.current, paraRef.current, ctaRef.current], { autoAlpha: 1, y: 0 });
          const microItems = [microItem1Ref.current, microItem2Ref.current, microItem3Ref.current].filter(Boolean);
          gsap.set(microItems, { autoAlpha: 1, y: 0 });
          if (headerRef.current) gsap.set(headerRef.current, { autoAlpha: 1 });
          return;
        }
        ...
        document.fonts.ready.then(() => {
          ...
          try {
            split = new SplitText(headerRef.current!, { type: 'lines', wordsClass: 'lines' });
          ...
          if (split?.lines?.length) {
            ...
            tl.to(headerRef.current, { autoAlpha: 1, duration: 0.5 }, 0.1);
          }
  ...
  return (
    <section
      ...
      style={{ width: '100vw', maxWidth: 'none', marginLeft: 'calc(-50vw + 50%)' }}
      id="inicio"
    >
